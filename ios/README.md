...
In Xcode, select your connected iPhone, configure signing if prompted, then click Run. On the iPhone, trust the developer profile if iOS asks.
...
- `ios/App/App/public/` is generated by `npm run ios:sync` from `web/dist`.
- `web/` contains the React + Vite web layer and shared static assets.
- `capacitor.config.json` points Capacitor at `web/dist`.

The iOS app is not primarily running the React app in a WebView. Capacitor is still present for project packaging and syncing web assets, but the main iPhone UI is native SwiftUI.
